...
每日信件預設寄出科技產業優先的前 50 名，並列出正式收盤價、首次入選價與入選至今累積漲跌、單日及近三日漲跌、三大分數、資料完整度、排名理由與主要風險。連續三次以上入選者會以紅字提醒；同一市場交易日只寄一次，追蹤紀錄保存在 `backend/data/email_report_history.json`。若晚間因上市、上櫃資料尚未同步而無法寄出，星期二至星期六 09:00 會自動補寄前一個尚未寄送的交易日。
